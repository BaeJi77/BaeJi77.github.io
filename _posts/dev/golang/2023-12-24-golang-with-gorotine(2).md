...
리소스를 생성하고 릴리즈하는데 있어서 차이가 존재합니다. 고루틴같은 경우 실제 os에서 관리되는 리소스가 아니다보니 os에 영역에 접근할 필요가 없습니다. 그리고 os쓰레드에 비해서 훨씬 적은 사이즈로 만들 수 있다. 
...
하지만 고루틴 같은 경우 실질적으로 os에서 동작하는 context switching이 아닌 user level에서 context switching이 일어나기 때문에 훨씬 더 적은 데이터를 저장하고 로드하게 됩니다. 실제로 적은 데이터를 관리하고 그것과 함께 os 영역과 연관성이 줄어들기에 일단적인 thread에 비해서 빠르게 context switching을 할 수 있다.
