...
1개의 process에서 여러개의 thread가 존재가능. Data, code, heap영역은 공유하여서 사용하고 stack과 register는 thread별로 가지고 있는다. Process보다 context switch가 빠르며(register와 stack만 바꾸면 되니까), 생성시에도 빠른 속도가 있다. 

- 질문) 왜 각 thread는 스택영역을 따로 가지고 있을까? (생각해보기)
